...
def generate_session_name():
    # 获取当前时间 %Y-%m-%d_%H-%M-%S %Y年 %m 月 %d 日 %H小时 %M分 %S秒
    ...
    if os.path.exists("sessions"):
        ...
        for filename in file_list:
            if filename.endswith(".json"):
                session_list.append(filename[:-5])#切片，获取文件名
...
if "nick_name" not in st.session_state:
    st.session_state.nick_name = "小宝贝"
# 性格
...
client = OpenAI(
    api_key=os.environ.get('sk-3f7c04cec344449e8352ae4f48ed54c8'),
...
with st.sidebar:
    ...
    for session in session_list:
        ...
        with col1:
           ...
           # width="stretch" 占用副容器的宽度，不受内容空间的控制
           # streamlit:type=primary/secondary 按钮类型:红色/透明
...
if prompt: # 字符串会自动转换为布尔值, 如果字符串非空, 则为True; ""否则为False
    ...
    full_response = ""      # 保存大模型返回的结果
    for chunk in response:  # 大模型返回的结果是一个流式输出, 每次返回一个片段
        if chunk.choices[0].delta.content is not None:  # 如果有内容
            content = chunk.choices[0].delta.content    # 获取内容
            full_response += content                    # 追加内容
            response_message.chat_message("assistant").write(full_response) # 输出大模型返回的结果
